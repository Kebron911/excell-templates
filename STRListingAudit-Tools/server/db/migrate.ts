 ...
 * deploy hooks.
 *
 * Reads MYSQL_* env vars from the local environment / .env.local. Caller is
...
async function main() {
  ...
  console.log(
    `[migrate] applying schema.sql against ${process.env.MYSQL_DATABASE ?? 'strlistingaudit'}…`,
  );
  try {
    await conn.query(sql);
    console.log(
      '[migrate] ok — three tables present: audit_runs, rate_limits, email_verifications',
    );
  } finally {
    await conn.end();
  }
