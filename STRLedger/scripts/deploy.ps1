# Deploy STRLedger static dist/ to Hostinger via SFTP (SSH key auth).
#
# Mirrors STRBuyers-Tools/scripts/deploy.ps1 with one critical addition:
# a **draft-status safety gate**. STRLedger is the parent-brand site
# (thestrledger.com) and currently has a separate Hostinger deploy
# serving live customers. The scaffold in this repo is NOT yet feature-
# complete vs the live site (see STRLedger/DEPLOY-STATUS.md).
#
# This script REFUSES to upload as long as DEPLOY-STATUS.md still
# contains the literal "DRAFT - DO NOT DEPLOY" marker. To unlock:
#   1) Verify feature parity against the live site (catalog, /free/47-deductions
#      form wiring, Etsy buy buttons, blog strategy).
#   2) Update DEPLOY-STATUS.md to mark status as "READY - feature parity".
#   3) Commit and push that change.
#   4) Re-run with -ConfirmReplaceLive to bypass the soft warning.
#
# Usage (from STRLedger/):
#   .\scripts\deploy.ps1                       # default: blocked while DRAFT
#   .\scripts\deploy.ps1 -Verify               # smoke a deployed site (no upload)
#   .\scripts\deploy.ps1 -ConfirmReplaceLive   # explicit ack you'll replace the live site
#   .\scripts\deploy.ps1 -SkipBackup -Force    # advanced; bypass backup + hash idempotence
#
# Requirements:
#   - SSH key at STRLEDGER_SSH_KEY_PATH (defaults to the cluster key
#     C:\Users\Kebron\.ssh\hostinger_ed25519). Key must be authorized on
#     the Hostinger account that owns thestrledger.com.
#   - Either WinSCP .NET assembly OR OpenSSH scp.exe + ssh.exe on PATH.
#   - dist/ already built (`pnpm build` from STRLedger/).
#
# Idempotent: dist-hash compared against .deploy-last-hash; skips upload
# if unchanged unless -Force.
#
...
param(
  ...
  [switch]$Verify,
  [switch]$Force,
  [switch]$ConfirmReplaceLive,
  [string]$Domain = 'thestrledger.com'
)

$ErrorActionPreference = 'Stop'

# --- Resolve paths ---
$projectRoot = Split-Path -Parent $PSScriptRoot       # .../STRLedger
$distDir     = Join-Path $projectRoot 'dist'
$hashFile    = Join-Path $projectRoot 'scripts\.deploy-last-hash'
$statusFile  = Join-Path $projectRoot 'DEPLOY-STATUS.md'
$secretsEnv  = 'C:\Users\Kebron\Desktop\Claude OS\.secrets\hostinger.env'

# --- DRAFT-status safety gate ---
# Refuse to upload as long as DEPLOY-STATUS.md still marks the scaffold
# as DRAFT. -Verify mode (no upload) is exempt because verifying the
# live site is safe.
if (-not $Verify) {
  if (-not (Test-Path $statusFile)) {
    Write-Warning "DEPLOY-STATUS.md not found at $statusFile."
    Write-Warning "If you deleted it intentionally to ship, re-run with -ConfirmReplaceLive."
    if (-not $ConfirmReplaceLive) { throw "Aborted: missing DEPLOY-STATUS.md and no -ConfirmReplaceLive flag." }
  } else {
    $statusBody = Get-Content $statusFile -Raw
    if ($statusBody -match 'DRAFT\s*[-—]?\s*DO NOT DEPLOY') {
      Write-Output ""
      Write-Output "=== DEPLOY BLOCKED ==="
      Write-Output ""
      Write-Output "STRLedger/DEPLOY-STATUS.md still marks this scaffold as DRAFT."
      Write-Output "Deploying now will replace the live thestrledger.com with an"
      Write-Output "incomplete catalog (review feature parity in DEPLOY-STATUS.md)."
      Write-Output ""
      Write-Output "To unlock the deploy:"
      Write-Output "  1) Read STRLedger/DEPLOY-STATUS.md and close every catalog/parity gap."
      Write-Output "  2) Edit DEPLOY-STATUS.md - replace 'DRAFT - DO NOT DEPLOY' with 'READY - feature parity verified <date>'."
      Write-Output "  3) Commit and push."
      Write-Output "  4) Re-run: .\scripts\deploy.ps1 -ConfirmReplaceLive"
      Write-Output ""
      if (-not $ConfirmReplaceLive) {
        throw "Aborted: scaffold marked DRAFT in DEPLOY-STATUS.md. Use -ConfirmReplaceLive to bypass after closing parity gaps."
      }
      Write-Warning "-ConfirmReplaceLive bypass acknowledged. Proceeding despite DRAFT marker."
    }
  }
}
...
$sshHost  = $cfg.STRLEDGER_SSH_HOST
$sshUser  = $cfg.STRLEDGER_SSH_USER
$sshPort  = $cfg.STRLEDGER_SSH_PORT
$keyPath  = $cfg.STRLEDGER_SSH_KEY_PATH
$docRoot  = $cfg.STRLEDGER_DOC_ROOT

if (-not $sshHost -or -not $sshUser -or -not $sshPort -or -not $keyPath -or -not $docRoot) {
  throw "Missing STRLEDGER_SSH_* vars in $secretsEnv (HOST/USER/PORT/KEY_PATH/DOC_ROOT all required)"
}
if (-not (Test-Path $keyPath)) {
  throw "SSH key not found at STRLEDGER_SSH_KEY_PATH = '$keyPath'"
}

# --- Smoke verify ---
function Test-Site {
  $checks = @(
    @{ url = "https://$Domain/";                            needle = 'STR Ledger' },
    @{ url = "https://$Domain/products/";                   needle = 'workbook' },
    @{ url = "https://$Domain/products/TAX-001/";           needle = 'Mileage Log' },
    @{ url = "https://$Domain/products/ACQ-001/";           needle = 'Deal Analyzer' },
    @{ url = "https://$Domain/free/47-deductions/";         needle = 'deduction' },
    @{ url = "https://$Domain/about/";                      needle = 'STR Ledger' },
