...
test.describe('Break-even calculator', () => {
  test('loads with feasible defaults, shows break-even occupancy', async ({ page }) => {
    ...
    await expect(page.getByText(/Break-even nights/i)).toBeVisible();
    await expect(page.getByText(/Break-even occupancy/i)).toBeVisible();
  });
  ...
  test('flags infeasible scenario when ADR is too low', async ({ page }) => {
    ...
    await expect(page.getByText(/Not feasible at this ADR/i)).toBeVisible();
  });
