...
| **Anthropic (listingaudit)** | `ANTHROPIC_API_KEY` | `STRListingAudit-Tools/.env.local` | n/a — models `claude-sonnet-4-5` (synth) + `claude-haiku-4-5` (per-dim) | ✅ set (verified 2026-05-16) — Phase 3 scorecard engine wired |
| **Apify (listingaudit)** | `APIFY_TOKEN`, `APIFY_AIRBNB_ACTOR` | `STRListingAudit-Tools/.env.local` | apify.com console | ✅ set (verified 2026-05-16) — actor `tri_angle/airbnb-scraper` |
| **Admin token (listingaudit)** | `ADMIN_TOKEN` | `STRListingAudit-Tools/.env.local` (local) + GitHub Actions secret `LISTINGAUDIT_ADMIN_TOKEN` (prod) | — | ✅ set 2026-05-16 (32-byte random) — gates `POST /api/scrape` debug endpoint |
| **MySQL (listingaudit)** | `MYSQL_HOST/PORT/USER/PASSWORD/DATABASE` | `STRListingAudit-Tools/.env.local` | Hostinger Business MySQL — DB `strlistingaudit` (open question: shared instance with strguests or new?) | ⏳ pending — Hostinger DB provisioning decision still open in STATE.md |
| **Email verify HMAC (listingaudit)** | `EMAIL_VERIFY_SECRET` | `STRListingAudit-Tools/.env.local` | — | ⏳ pending — only needed for Phase 4b (deferred email-gated PDF). Not a v0.1 launch blocker. |
| **IP hash salt (listingaudit)** | `IP_HASH_SALT` | `STRListingAudit-Tools/.env.local` (local) + GitHub Actions secret `LISTINGAUDIT_IP_HASH_SALT` (prod) | — | ✅ set 2026-05-16 (32-byte random) — rate-limit IP hashing |
| **GA4 (listingaudit)** | `PUBLIC_GA4_ID` (build-time) | GitHub Actions secret `LISTINGAUDIT_GA4_ID` (prod, written to `.env` at build); `STRListingAudit-Tools/.env.local` empty for local dev | — | ✅ set in Actions 2026-05-15 (deploy.yml writes to `.env` per build) |
...
Last updated: 2026-05-16 (listingaudit.tools: marked Anthropic, Apify, GA4 as ✅ set after audit revealed stale ⏳ status. Generated + stored IP_HASH_SALT and ADMIN_TOKEN in both `.env.local` and GitHub Actions secrets `LISTINGAUDIT_IP_HASH_SALT` / `LISTINGAUDIT_ADMIN_TOKEN`. Remaining ⏳: MySQL prod provisioning (open decision), EMAIL_VERIFY_SECRET (Phase 4b only)).
