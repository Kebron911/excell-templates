...
`templates/_delivery/_shared/etsy-upgrade-insert.pdf`

This single PDF is re-used in every Etsy listing (not per-SKU). When brand name or domain changes, update once and re-upload to all 5+ listings.

## Alternative variant — per-SKU upgrade matched

Once sales ramp, consider per-SKU variants of this PDF where the "Upgrade to full version" CTA is specific to that template's upgrade path. Higher conversion, but more maintenance. Defer until Month 2.
