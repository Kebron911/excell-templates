 ...
 *
 * Used by every tool. Numbers render with `font-variant-numeric: tabular-nums`
 * via the `.num` / `.font-mono` class on the rendering element — these helpers
 * return strings only.
 *
 * Non-finite inputs (NaN, Infinity) return the em-dash "—" so tools can render
 * gracefully when an input is missing or a division-by-zero occurs.
 *
 * Identical to strhost.tools format library.
 */
...
export function formatPercent(value: number, opts: FormatPercentOptions = {}): string {
  ...
  // Disambiguation: values in (0, 1] are decimal percent; values > 1 are
  // already-percent. Special case 1 → "100%". Negatives use absolute value.
