# GST-001 Welcome Book v2 — Tool Redesign Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Rebuild GST-001 Welcome Book as a wizard-style Excel tool (no macros) with Cover hero, card-grouped input sections, Review & Print live preview, and 2-file DEMO/BLANK bundle.

**Architecture:** New builder `build_welcome_book_v2.py` that constructs 12 tabs (Start + 8 input sections + Review & Print + Bonus + Host Notes). Per-tab functions use shared helpers added to `brand_config.py` (pseudo-button, card header/body, navigation). A single `build_workbook(out_path, variant)` entry-point generates both DEMO (filled) and BLANK (cleared) xlsx files from the same code path with a variant flag gating sample data injection. HYPERLINK formulas + cross-sheet references + COUNTA progress — all native `.xlsx`, no macros.

**Tech Stack:** Python 3.10+, `openpyxl==3.1.5`, `brand_config.py` (existing, extended), Excel 2016+ / Excel 365 / Google Sheets as target runtimes.

**Parent design:** [../specs/2026-04-23-welcome-book-v2-tool-redesign.md](../specs/2026-04-23-welcome-book-v2-tool-redesign.md)
**Related existing artifacts (unchanged):**
- `templates/_briefs/GST-001-welcome-book.md` — brief (still source of truth for inputs)
- `templates/_briefs/GST-001-welcome-book-spec.md` — v1 sheet spec (partially superseded; v2 uses new shape)
- `copy/etsy-listings/GST-001-welcome-book.md` — listing (will get 3 new bullets in Task 10)
- `templates/_delivery/GST-001-welcome-book/` — delivery artifacts (unchanged)
...
### Create
```
templates/_build/build_welcome_book_v2.py       # new builder (~800 LOC)
```

### Modify
```
templates/_build/brand_config.py                # add 4 helpers (pseudo_button, card_header, card_body, section_header_band)
copy/etsy-listings/GST-001-welcome-book.md      # 3 new bullets about v2 features
```

### Delete (in final cleanup task)
```
templates/_build/build_welcome_book.py          # v1 builder superseded by v2
templates/_masters/GST-001-welcome-book.xlsx    # v1 output superseded by DEMO + BLANK
```

### Generated (not committed by Python — committed via git add after build)
```
templates/_masters/GST-001-welcome-book-DEMO.xlsx    # filled sample data
templates/_masters/GST-001-welcome-book-BLANK.xlsx   # cleared inputs
templates/_delivery/GST-001-welcome-book/GST-001-sample-output.pdf   # Daniel prints from DEMO (Task 11)
```

---

## Task 1: Extend brand_config.py with wizard helpers

**Files:**
- Modify: `templates/_build/brand_config.py`

**Acceptance:** 4 new helpers importable (`pseudo_button`, `card_header`, `card_body_fill`, `section_header_band`). Existing helpers unchanged.

- [ ] **Step 1: Open `templates/_build/brand_config.py` and locate the bottom of the file (after `add_upgrade_banner`)**

Run: `grep -n "^def " templates/_build/brand_config.py`
Expected: lists `apply_brand_header`, `header_row_style`, `input_cell_style`, `formula_cell_style`, `apply_style`, `set_col_widths`, `add_upgrade_banner` — roughly in that order.

- [ ] **Step 2: Append 4 new helper functions at the bottom of `brand_config.py`**
...
- [ ] **Step 3: Run verification — module imports cleanly and new helpers are callable**

Run:
```bash
cd "C:/Users/Kebron/Desktop/Claude OS/Excell-Templates"
python -c "import sys; sys.path.insert(0, 'templates/_build'); import brand_config as bc; assert callable(bc.pseudo_button); assert callable(bc.card_header); assert callable(bc.card_body_fill); assert callable(bc.section_header_band); print('OK: 4 helpers importable')"
```

Expected: `OK: 4 helpers importable`

- [ ] **Step 4: Smoke-test the helpers produce valid workbook output**

Run:
```bash
cd "C:/Users/Kebron/Desktop/Claude OS/Excell-Templates"
python -c "
import sys; sys.path.insert(0, 'templates/_build')
import brand_config as bc
from openpyxl import Workbook
wb = Workbook()
ws = wb.active
bc.pseudo_button(ws, 'A1', 'C2', 'Test', \"'Sheet'!A1\", variant='primary')
bc.card_header(ws, 4, ('A', 'L'), 'Test card')
bc.card_body_fill(ws, 5, 8, ('A', 'L'), border=True)
bc.section_header_band(ws, 1, 8, 'Test Section', 'Test subtitle', '', 'NextTab')
wb.save('/tmp/test.xlsx' if sys.platform != 'win32' else 'C:/Users/Kebron/AppData/Local/Temp/test.xlsx')
print('helpers produce valid output')
"
```

Expected: `helpers produce valid output`

- [ ] **Step 5: Commit**

```bash
cd "C:/Users/Kebron/Desktop/Claude OS/Excell-Templates"
git add templates/_build/brand_config.py
git commit -m "build: add wizard helpers to brand_config (pseudo_button, card_header, card_body_fill, section_header_band)"
```

---

## Task 2: Scaffold build_welcome_book_v2.py

**Files:**
- Create: `templates/_build/build_welcome_book_v2.py`

**Acceptance:** File exists with imports, constants, data structures, and `build_workbook` + `main` stubs. Runs without error (produces empty 12-tab workbook).

- [ ] **Step 1: Create `templates/_build/build_welcome_book_v2.py` with scaffolding**
...
SAMPLE = {
    ...
    # Local Guide samples — first of each category
    "Local": [
...
- [ ] **Step 2: Run the scaffold — should produce 2 empty-shelled files**
...
Expected output:
```
Saved: .../_masters/GST-001-welcome-book-DEMO.xlsx
Saved: .../_masters/GST-001-welcome-book-BLANK.xlsx
```

- [ ] **Step 3: Smoke test both files have 12 tabs in correct order**
...
for path in ['templates/_masters/GST-001-welcome-book-DEMO.xlsx', 'templates/_masters/GST-001-welcome-book-BLANK.xlsx']:
    wb = load_workbook(path)
    expected = ['Start', 'Property', 'Arrival', 'WiFi + Tech', 'House Rules', 'Local Guide', 'Trash', 'Departure', 'Emergency', 'Review & Print', 'Bonus', '× Host Notes']
    assert wb.sheetnames == expected, f'{path}: got {wb.sheetnames}'
    print(f'OK: {path}')
"
```
...
```bash
cd "C:/Users/Kebron/Desktop/Claude OS/Excell-Templates"
git add templates/_build/build_welcome_book_v2.py templates/_masters/GST-001-welcome-book-DEMO.xlsx templates/_masters/GST-001-welcome-book-BLANK.xlsx
git commit -m "build: v2 Welcome Book scaffold — 12-tab shell + DEMO/BLANK variants"
```

---

## Task 3: Implement Start tab (Cover)

**Files:**
- Modify: `templates/_build/build_welcome_book_v2.py` (replace `build_start_tab` function body)

**Acceptance:** Start tab renders hero band + 3-card + Quick Start card + Get Started button + progress dashboard + footer. Progress formulas use COUNTA across all 8 input tab ranges.

- [ ] **Step 1: Replace `build_start_tab` stub with full implementation**

In `templates/_build/build_welcome_book_v2.py`, replace the `build_start_tab` stub function with:
...
- [ ] **Step 2: Rebuild both files and verify Start tab structure**
...
- [ ] **Step 3: Verify Start tab formulas + hero text**
...
ws = wb['Start']
print('A4 title:', ws['A4'].value)
print('A5 subtitle:', ws['A5'].value)
print('A11 zone2 heading:', ws['A11'].value)
print('A23 quickstart heading:', ws['A23'].value)
print('A30 Get Started button:', ws['A30'].value[:80])
print('G36 progress:', ws['G36'].value[:60])
print('A38 status row 1:', ws['A38'].value)
print('G38 status formula:', ws['G38'].value[:60])
print('K38 arrow link:', ws['K38'].value)
"
```

Expected: "Welcome Book", "Welcome books that earn 5-stars.", "What you'll build..." etc., plus the COUNTA-based progress formula on G36 and status+arrow formulas on row 38.

- [ ] **Step 4: Commit**

```bash
cd "C:/Users/Kebron/Desktop/Claude OS/Excell-Templates"
git add templates/_build/build_welcome_book_v2.py templates/_masters/
git commit -m "build: v2 Welcome Book Start tab — hero + 3-card + Quick Start + Get Started + progress dashboard"
```

---

## Task 4: Implement input-tab card-template helper

**Files:**
- Modify: `templates/_build/build_welcome_book_v2.py` (add `build_input_tab` helper + card dataclass)

**Acceptance:** `build_input_tab` function exists and can be called with a section config; produces a tab with step header band, instruction strip, N cards, and section footer. Test-call it on a dummy tab to verify layout.

- [ ] **Step 1: Add card dataclass + `build_input_tab` helper near the top of the file (after `_sample_or_blank`)**
...
- [ ] **Step 2: Verify dataclass + function imports cleanly**

Run:
```bash
cd "C:/Users/Kebron/Desktop/Claude OS/Excell-Templates/templates/_build"
python -c "
import build_welcome_book_v2 as mod
print('Card dataclass:', mod.Card)
print('build_input_tab callable:', callable(mod.build_input_tab))
print('OK')
"
```

Expected: `Card dataclass: <class 'Card'>`, `build_input_tab callable: True`, `OK`.
...
git commit -m "build: v2 Welcome Book — add build_input_tab helper + Card dataclass"
```

---

## Task 5: Implement all 8 input-section tabs

**Files:**
- Modify: `templates/_build/build_welcome_book_v2.py` (replace 8 stub functions with real implementations using `build_input_tab`)

**Acceptance:** All 8 input tabs render with card layout; sample data populates in DEMO variant; BLANK variant has empty inputs; dropdowns work on House Rules/Arrival/Trash.

- [ ] **Step 1: Replace the 8 stub functions with real implementations**

Replace `build_property_tab` through `build_emergency_tab` with:
...
def build_local_tab(wb, variant):
    """Tab 5 — Local Guide. 20 category rows × 4 input cols = 80 inputs.
...
from openpyxl import load_workbook
wb = load_workbook('templates/_masters/GST-001-welcome-book-DEMO.xlsx')
# Sample values should exist in DEMO
assert wb['Property']['D10'].value == 'Smokies Ridge Cabin', f\"Property name: {wb['Property']['D10'].value}\"
assert 'Smart lock' in str(wb['Arrival']['D10'].value) or wb['Arrival']['D11'].value == 'Smart lock', 'Arrival entry_method'
assert wb['WiFi + Tech']['D11'].value == 'welcome2024', f\"WiFi password: {wb['WiFi + Tech']['D11'].value}\"
assert 'CALL 911' in wb['Emergency']['A7'].value
assert wb['Emergency']['D13'].value == '1-800-222-1222', 'poison control hardcoded'
# Verify BLANK variant
wb2 = load_workbook('templates/_masters/GST-001-welcome-book-BLANK.xlsx')
# Input cells should be None/empty in BLANK (cells exist but values are empty)
prop_name = wb2['Property']['D10'].value
assert prop_name is None or prop_name == '', f'BLANK Property name should be empty, got: {prop_name!r}'
print('OK: 8 input tabs render with correct sample/blank behavior')
"
```

(Note: exact row/col for the first input may differ slightly based on card header rows. Adjust the asserts to match actual layout after first run — print the cell values first, then lock the asserts.)

- [ ] **Step 3: Daniel QA — open DEMO in Excel on Windows, tab through 8 sections**

Daniel opens `templates/_masters/GST-001-welcome-book-DEMO.xlsx` and walks through:
- [ ] Property → Arrival → WiFi + Tech → House Rules → Local Guide → Trash → Departure → Emergency via Next buttons (hyperlinks work)
- [ ] Back buttons also work
- [ ] Dropdowns: Property type, Arrival entry method, all 4 House Rules dropdowns, Trash pickup day
- [ ] Emergency tab shows big red 911 block + cross-sheet formula `=Property!B7` pulls the host phone
- [ ] Local Guide has 20 category rows, landscape print orientation

If anything fails: fix in build script, re-run.
...
cd "C:/Users/Kebron/Desktop/Claude OS/Excell-Templates"
git add templates/_build/build_welcome_book_v2.py templates/_masters/
git commit -m "build: v2 Welcome Book — 8 input tabs with card-grouped layout, dropdowns, cross-sheet formulas"
```

---

## Task 6: Implement Review & Print tab

**Files:**
- Modify: `templates/_build/build_welcome_book_v2.py` (replace `build_review_print_tab` stub)

**Acceptance:** Review & Print tab has review header, readiness dashboard (3 cards), Generate PDF pseudo-button, and 3-page live preview pulling from all 8 input tabs. Print area set to produce 3-page letter-portrait PDF on Ctrl+P.

- [ ] **Step 1: Replace `build_review_print_tab` stub with full implementation**
...
def build_review_print_tab(wb, variant):
    ...
    c.alignment = Alignment(horizontal="center", vertical="center")
    # Conditional formatting on status card (simplified: visual-only via text)
    ...
    for first, last in [("A", "D"), ("E", "H"), ("I", "L")]:
        from openpyxl.utils.cell import column_index_from_string as col_idx
        fc, lc = col_idx(first), col_idx(last)
        for r in range(9, 13):
            for c in range(fc, lc + 1):
                ...
                new_border = Border(
                    ...
                    left=gold_side if c == fc else existing.left,
                    right=gold_side if c == lc else existing.right,
                )
    ...
    # Property info block (rows 27-32)
    ...
    ws.page_breaks.rowBreaks.append(Break(id=46))
    ...
    ws.page_breaks.rowBreaks.append(Break(id=66))
...
wb = load_workbook('templates/_masters/GST-001-welcome-book-DEMO.xlsx')
ws = wb['Review & Print']
print('title:', ws['A4'].value)
print('completion formula:', ws['A10'].value[:80])
print('red flags formula:', ws['E10'].value[:80])
print('status formula:', ws['I10'].value[:80])
print('hero line:', ws['A24'].value[:60])
print('WiFi password preview:', ws['E40'].value)
print('page breaks:', [b.id for b in ws.page_breaks.rowBreaks])
print('print area:', ws.print_area)
"
```

Expected: print area `A24:L80`, 2 page breaks at rows 46 and 66, COUNTA formulas in dashboard cards, cross-sheet formulas in preview.

- [ ] **Step 3: Daniel opens DEMO in Excel, navigates to Review & Print tab, hits Ctrl+P**

Verify:
- [ ] 3-card readiness dashboard renders correctly
- [ ] Completion shows a percentage (DEMO should be ~100%)
- [ ] Red Flags shows 0 or 1 (all required fields are filled in DEMO)
- [ ] Status shows "READY"
- [ ] Generate PDF button is visible and styled
- [ ] 3-page preview below renders with actual values pulled from input tabs
- [ ] Ctrl+P → Print Preview shows 3 pages of letter-portrait content
- [ ] Save as PDF produces a 3-page PDF
...
cd "C:/Users/Kebron/Desktop/Claude OS/Excell-Templates"
git add templates/_build/build_welcome_book_v2.py templates/_masters/
git commit -m "build: v2 Welcome Book — Review & Print tab with live 3-page preview + readiness dashboard"
```

---

## Task 7: Implement Bonus tab

**Files:**
- Modify: `templates/_build/build_welcome_book_v2.py` (replace `build_bonus_tab` stub)

**Acceptance:** Bonus tab has pre-written Airbnb listing copy in 3 sections (House Rules, Check-in, Before-you-leave reminder). Content is static (no inputs, no formulas). Buyers copy-paste into their Airbnb listing.

- [ ] **Step 1: Replace `build_bonus_tab` stub**
...
python -c "
from openpyxl import load_workbook
wb = load_workbook('../_masters/GST-001-welcome-book-DEMO.xlsx')
ws = wb['Bonus']
print('title:', ws['A4'].value)
print('block 1 preview:', str(ws['A10'].value)[:80])
"
```
...
cd "C:/Users/Kebron/Desktop/Claude OS/Excell-Templates"
git add templates/_build/build_welcome_book_v2.py templates/_masters/
git commit -m "build: v2 Welcome Book — Bonus tab with 3 pre-written Airbnb listing blocks"
```

---

## Task 8: Implement Host Notes quarantine tab

**Files:**
- Modify: `templates/_build/build_welcome_book_v2.py` (replace `build_host_notes_tab` stub)

**Acceptance:** × Host Notes tab has red warning header, card-grouped private inputs, print area set to A1:A1 (1 cell) for safety, clay rose tab color.

- [ ] **Step 1: Replace `build_host_notes_tab` stub**
...
def build_host_notes_tab(wb, variant):
    ...
    # Cards — same structure as Emergency tab
    parchment = PatternFill("solid", fgColor=COLOR_BG_LIGHT)
    current_row = 7
    for hdr, rows in [
        ("Cleaners & Handyman", [
...
wb = load_workbook('../_masters/GST-001-welcome-book-DEMO.xlsx')
ws = wb['× Host Notes']
print('warning:', ws['A1'].value)
print('print area:', ws.print_area)
print('tab color:', ws.sheet_properties.tabColor.rgb if ws.sheet_properties.tabColor else None)
"
```

Expected: warning text, print area `A1:A1`, tab color includes `B5725E`.
...
cd "C:/Users/Kebron/Desktop/Claude OS/Excell-Templates"
git add templates/_build/build_welcome_book_v2.py templates/_masters/
git commit -m "build: v2 Welcome Book — × Host Notes tab quarantined with red warning + A1:A1 print area"
```

---

## Task 9: End-to-end smoke + round-trip test

**Files:** no file changes

**Acceptance:** Both DEMO and BLANK files load cleanly; DEMO has sample data visible in preview; BLANK has empty fields showing "(not set)" in preview; all hyperlinks point to valid tab targets; progress formula on Start tab evaluates to a sensible percentage.

- [ ] **Step 1: Run comprehensive validation script**
...
for variant, path in [('DEMO', 'templates/_masters/GST-001-welcome-book-DEMO.xlsx'),
                      ('BLANK', 'templates/_masters/GST-001-welcome-book-BLANK.xlsx')]:
    print(f'=== {variant} ===')
    wb = load_workbook(path)
    assert len(wb.sheetnames) == 12, f'{variant}: got {len(wb.sheetnames)} tabs'
    expected = ['Start', 'Property', 'Arrival', 'WiFi + Tech', 'House Rules', 'Local Guide', 'Trash', 'Departure', 'Emergency', 'Review & Print', 'Bonus', '× Host Notes']
    assert wb.sheetnames == expected, f'{variant}: tab order wrong'

    # Start tab — hero + button + progress
    start = wb['Start']
    assert start['A4'].value == 'Welcome Book'
    assert '=HYPERLINK' in str(start['A30'].value)
    assert '=TEXT' in str(start['G36'].value) and 'COUNTA' in str(start['G36'].value)

    # Review & Print — dashboard + preview
    rp = wb['Review & Print']
    assert '=TEXT' in str(rp['A10'].value) and 'COUNTA' in str(rp['A10'].value)
    assert rp['A24'].value.startswith('=CONCATENATE')
    print(f'  {variant} Start hero + Review dashboard + preview formulas OK')

    # Host Notes — quarantine
    hn = wb['× Host Notes']
    assert hn.print_area == 'A1:A1', f'{variant}: Host Notes print area is {hn.print_area}'
    assert 'PRIVATE' in hn['A1'].value
    print(f'  {variant} Host Notes print area = A1:A1, warning present')
...
```bash
cd "C:/Users/Kebron/Desktop/Claude OS/Excell-Templates"
ls -lh templates/_masters/GST-001-welcome-book-DEMO.xlsx templates/_masters/GST-001-welcome-book-BLANK.xlsx
```

Expected: both under 100 KB. If over 500 KB, something's wrong (likely excessive styling loops creating artifact bloat).

- [ ] **Step 3: Daniel opens both files in Excel 365 on Windows, does a full walkthrough**

Walk through the DEMO:
- [ ] Start tab hero renders; Get Started button navigates to Property!A5 on click
- [ ] Each input tab has navy step header with Back/Next buttons that work
- [ ] Cards render with gold-soft header strips + parchment body + gold border
- [ ] All dropdowns work (Property type, Arrival entry, Rules × 4, Trash pickup day)
- [ ] Local Guide shows 20 rows landscape
- [ ] Emergency tab has big red 911 block + card-grouped contacts + host phone formula
- [ ] Review & Print: completion shows 100%, red flags 0, status "READY", 3-page preview renders
- [ ] Ctrl+P → "Microsoft Print to PDF" → produces 3-page PDF
- [ ] Host Notes has red warning; Ctrl+P produces near-empty page
- [ ] Bonus tab has 3 pre-written blocks; select-all + copy works

Walk through BLANK:
- [ ] Every input cell is empty
- [ ] Progress on Start tab shows 0% or very low
- [ ] Review & Print preview shows "(not set)" for every pulled field (except hardcoded like Poison Control)
- [ ] Red Flags shows 10 (all required fields empty)
- [ ] Status shows "NEEDS WORK"

- [ ] **Step 4: If Daniel finds issues, fix + rebuild**

For each issue: identify the tab + function + row/col, fix in `build_welcome_book_v2.py`, rebuild, re-verify.
...
cd "C:/Users/Kebron/Desktop/Claude OS/Excell-Templates"
git add templates/_build/build_welcome_book_v2.py templates/_masters/
git commit -m "build: v2 Welcome Book — QA fixes from Daniel walkthrough"
```

---

## Task 10: Update Etsy listing copy

**Files:**
- Modify: `copy/etsy-listings/GST-001-welcome-book.md`

**Acceptance:** listing reflects v2 features (2-file bundle + live 3-page preview + bonus listing copy) in Description section + files-attached list. Title + tags unchanged (SEO-optimized already).

- [ ] **Step 1: Read current listing**

Run:
```bash
grep -n "WHAT'S INCLUDED" copy/etsy-listings/GST-001-welcome-book.md | head -5
```

- [ ] **Step 2: Locate the WHAT'S INCLUDED section and add 3 new bullets at the top**

Edit `copy/etsy-listings/GST-001-welcome-book.md`. Find this existing block in the description:
...
Change the bullets to add the new v2 features. Replace:
...
- [ ] **Step 3: Update the "Files attached to Etsy" block**

Find the existing file list section (around line 196 in v1 listing) and replace with:
...
(The old file list referenced a single xlsx — `templates/_masters/GST-001-welcome-book.xlsx` — which is deleted in Task 11.)

- [ ] **Step 4: Update the "Status" field at the top of the listing**

Find: `**Status:** Ready to publish.` (or similar)
...
- [ ] **Step 5: Add iteration log entry**

Find the iteration log at the bottom of the file. Add:
...
git commit -m "copy: GST-001 listing updated for v2 — DEMO+BLANK bundle, live preview, bonus listing copy"
```

---

## Task 11: Cleanup — delete v1 build + old xlsx; generate sample PDF

**Files:**
- Delete: `templates/_build/build_welcome_book.py`
- Delete: `templates/_masters/GST-001-welcome-book.xlsx`
- Generate (Daniel): `templates/_delivery/GST-001-welcome-book/GST-001-sample-output.pdf`

**Acceptance:** v1 artifacts removed from repo. Sample PDF exists in the delivery folder (produced by printing the DEMO's Review & Print tab).

- [ ] **Step 1: Verify v2 files exist before deleting v1**

```bash
cd "C:/Users/Kebron/Desktop/Claude OS/Excell-Templates"
ls templates/_masters/GST-001-welcome-book-DEMO.xlsx templates/_masters/GST-001-welcome-book-BLANK.xlsx
```

Both must exist. If either is missing, stop — do not delete v1.

- [ ] **Step 2: Delete v1 build script**

```bash
cd "C:/Users/Kebron/Desktop/Claude OS/Excell-Templates"
git rm templates/_build/build_welcome_book.py
```

- [ ] **Step 3: Delete v1 xlsx master**

```bash
cd "C:/Users/Kebron/Desktop/Claude OS/Excell-Templates"
git rm templates/_masters/GST-001-welcome-book.xlsx
```
...
```bash
cd "C:/Users/Kebron/Desktop/Claude OS/Excell-Templates"
git commit -m "cleanup: remove v1 Welcome Book — superseded by DEMO + BLANK bundle"
```

- [ ] **Step 5: Daniel generates the sample PDF from the DEMO's Review & Print tab**

Daniel opens `templates/_masters/GST-001-welcome-book-DEMO.xlsx`:

1. Click the "Review & Print" tab (tab 10)
2. Press Ctrl+P (or File → Print)
3. From the Printer dropdown, select **"Microsoft Print to PDF"** (Windows) or **"Save as PDF"** (Mac)
4. Ensure orientation = Portrait, paper = Letter, margins = Narrow
5. Click Print
6. Save to: `C:/Users/Kebron/Desktop/Claude OS/Excell-Templates/templates/_delivery/GST-001-welcome-book/GST-001-sample-output.pdf`

- [ ] **Step 6: Verify the PDF**

Open the generated PDF. Confirm:
- [ ] 3 pages total
- [ ] Page 1: hero band "Welcome to Smokies Ridge Cabin" + host info + address + WiFi credentials in big bold
- [ ] Page 2: House Rules + Local Guide top 10
- [ ] Page 3: Trash + Checkout + Emergency 911 block

If the PDF is wrong (wrong number of pages, formulas show `#REF!`, text cut off): rebuild DEMO, re-print.

- [ ] **Step 7: Commit the sample PDF**

```bash
cd "C:/Users/Kebron/Desktop/Claude OS/Excell-Templates"
git add templates/_delivery/GST-001-welcome-book/GST-001-sample-output.pdf
git commit -m "deliver: GST-001 sample-output.pdf (3-page preview built from DEMO)"
```
...
Should show ~11 commits from this plan in sequence.

---

## Self-review notes

Ran against spec `docs/superpowers/specs/2026-04-23-welcome-book-v2-tool-redesign.md`:

**1. Spec coverage:**
- §2 Wizard shape (12 tabs) → Task 2 scaffolding defines TAB_NAMES list + all 12 build functions; Task 5 wires actual content
- §3 Cover tab → Task 3 implements all 6 zones (hero, 3-card, quickstart, Get Started, progress, footer)
- §4 Input tab template → Task 4 adds `build_input_tab` with Card dataclass; Task 5 instantiates for all 8 sections
- §5 Review & Print → Task 6 implements readiness dashboard + pseudo-button + 3-page live preview + page breaks
- §6 Host Notes quarantine → Task 8 implements red warning + A1:A1 print area
- §7 No-macro mechanism → every HYPERLINK() + COUNTA() + cross-sheet formula is in the relevant task's code
- §8 2-file bundle → Task 2 defines `build_workbook(out_path, variant)`; Task 5 uses `variant` flag on every sample data read
- §9 Saleability — F1 (DEMO + BLANK) in Task 2; F2 (live preview as Thumbnail 1) — not code, delivery decision (see listing update Task 10); F3 (Quickstart card) in Task 3 Zone 3; F4 (Bonus: Listing Copy) in Task 7; F7 (version footer) in Task 3 Zone 6
- §10 What stays the same → Task 10 updates listing without touching title/tags; brief + license unchanged
- §11 Acceptance criteria → Task 9 end-to-end validation script covers 12 of 13 checkboxes; sample PDF checkbox covered in Task 11

**2. Placeholder scan:** no TBD, TODO, "implement later", or "similar to Task N". Every task has full inline code.

**3. Type consistency:**
- `TAB_NAMES` list consistent throughout (Tasks 2, 3, 5, 6, 7, 8)
- `SAMPLE` dict keys match what Tasks 5, 6, 8 read
- `variant` parameter ("demo" / "blank") consistent across all build_*_tab functions
- Card dataclass used in Task 4 definition + Task 5 instantiations with matching signature
- Helpers added in Task 1 (`pseudo_button`, `card_header`, `card_body_fill`, `section_header_band`) imported in Task 2 and used in Tasks 3, 5, 6, 7, 8

**4. Known gaps / intentional deferrals:**
- Conditional formatting on Review & Print STATUS card text color is done via bold/color only — not a true conditional-format rule. Acceptable for v2; can upgrade in v2.1.
- Local Guide "top 10" in Review & Print preview pulls rows 10-19 regardless of whether they have content. Empty rows render as empty lines. Acceptable — buyer can order their 20 rows by priority.
- The ⚙ "find first blank cell" progress arrow (spec §7.4 E4-advanced) is simplified to always-to-A5 (E4-simple) per spec's explicit recommendation. Matches design decision.

---

**Plan complete. 11 tasks across ~1-2 days of Class A work + ~30 min Daniel QA + ~10 min Daniel PDF export.**
